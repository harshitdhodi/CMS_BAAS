...
```bash
curl https://branduntold.in/api/breadcrumbs/COLLECTION_ID/ANDROID_PHONES_ID \
  -H "Authorization: Bearer TOKEN"
```
...
### Step 1: Top-Level
```bash
curl -X POST https://branduntold.in/api/data/product_categories \
...
Save in `.env.local`:
```
NEXT_PUBLIC_API_URL=https://branduntold.in
AUTH_TOKEN=your_token_here

# For setup script
API_URL=https://branduntold.in
```
